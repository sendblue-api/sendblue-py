...
from sendblue_api import SendblueAPI, AsyncSendblueAPI, DefaultAioHttpClient
from sendblue_api._utils import is_dict
...
logging.getLogger("sendblue_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[SendblueAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with SendblueAPI(
        base_url=base_url, api_key=api_key, api_secret=api_secret, _strict_response_validation=strict
    ) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncSendblueAPI]:
